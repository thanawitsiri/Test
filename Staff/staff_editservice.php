...
?>
<?
$strSQL = "SELECT * FROM tb_service WHERE Service_ID = '".$_GET["Service_ID"]."' ";
$objQuery = mysql_query($strSQL);
$objResult = mysql_fetch_array($objQuery);
if(!$objResult)
{
echo "Not found Service_ID=".$_GET["Service_ID"];	
}
?>
...
<body>
<table width="962" height="626" border="0" >
  <tr>
  ...
  <tr>
    <td width="189" height="201" align="left" valign="top" bgcolor="#999999"><h2>
      ...
      <p></p></td>
    <td width="763" align="center" valign="top" bgcolor="#A9F5D0">
      <form id="form1" name="form1" method="post" action="staff___update_service.php?Service_ID=<?php echo $_GET["Service_ID"];?> ">
        <h2>แก้ไขข้อมูลค่าบริการ</h2>
        <table width="290" height="147" border="0">
          <tr>
            <td width="120">รหัสค่าบริการ  :</td>
            <td width="160"><?php echo $objResult["Service_ID"];?></td>
          </tr>
          <tr>
            <td>รายการ*          :</td>
            <td><input name="Service_Name" type="text" id="Service_Name" value="<?php echo $objResult["Service_Name"];?>" /></td>
          </tr>
          <tr>
            <td>หน่วยนับ*        :</td>
            <td><input name="Service_Unit" type="text" id="Service_Unit" value="<?php echo $objResult["Service_Unit"];?>" /></td>
          </tr>
          <tr>
            <td>ราคาต่อวัน*     :</td>
            <td><input name="Service_Price" type="text" id="Service_Price" align="right" value="<?php echo $objResult["Service_Price"];?>" size="5" />
            บาท</td>
          </tr>
          <tr>
            <td>สถานะ*           :</td>
            <td><select name="Service_Status" id="Service_Status">
              <option value="<?php echo $objResult["Service_Status"];?>" selected="selected"><?php if($objResult["Service_Status"] == 'Y'){ echo "ใช้งาน"; } elseif($objResult["Service_Status"] == 'N'){echo "ไม่ใช้งาน";} ?></option>
        ...
        <p>
          ...
          <input name="button" type="submit" id="button6" onclick="MM_validateForm('Service_Name','','R','Service_Unit','','R','Service_Price','','R');return document.MM_returnValue" value="บันทึก" />
