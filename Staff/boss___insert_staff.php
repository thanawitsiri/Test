...
?>
<?
	if($_POST[Emp_Level] == 2) 
	{ $level = 'เจ้าของกิจการ';
	}else { $level = 'พนักงาน';
	}
	if($_POST[Emp_Name] != NULL
	and $_POST[Emp_Phone] != NULL
	and $_POST[Emp_Email] != NULL
	and $_POST[Emp_Level] != NULL
	and $_POST[Emp_Pass] != NULL
	and $_POST[Emp_User] != NULL){
	$sql="INSERT INTO tb_emp (Emp_Name,Emp_Phone,Emp_Email,Emp_Level,Emp_Pass,Emp_User,Emp_Position,Emp_Address) 
	VALUES ('".$_POST[Emp_Name]."','".$_POST[Emp_Phone]."','".$_POST[Emp_Email]."'
	,'".$_POST[Emp_Level]."','".$_POST[Emp_Pass]."','".$_POST[Emp_User]."','".$level."','".$_POST[Emp_Address]."')";
	$query=mysql_query($sql) or die(mysql_error());
	if($query)
	{
		$str="สร้างข้อมูลพนักงานเสร็จสิ้น";
	} 
	else
	{
	$str="สร้างข้อมูลพนักงานล้มเหลว";
	}
}
?>
...
<table width="1163" height="671" border="0" >
  ...
  <tr>
    <td width="189" height="201" align="left" valign="top" bgcolor="#999999"><h2>
      ...
      <input name="บันทึกการจอง" type="submit" id="บันทึกการจอง" onclick="MM_goToURL('parent','staff_showroom.php');return document.MM_returnValue" value="            แสดงห้องพัก             " />
      <br />
      <a href="staff_room.html">
    ...
    <td width="964" align="center" valign="top" bgcolor="#CED8F6"><h4><? echo $str; ?></h4>
      <p><a href="boss_staff.php">กลับไปหน้าข้อมูลพนักงาน</a></p>
